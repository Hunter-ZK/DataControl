...
function Wait-HttpEndpoint([System.Diagnostics.Process]$Process,[string]$Url,[string]$Name,[string]$ErrorLog,[int]$TimeoutSeconds=25) {
  $deadline=(Get-Date).AddSeconds($TimeoutSeconds)
  while((Get-Date)-lt $deadline){$Process.Refresh();if($Process.HasExited){throw "$Name exited before becoming reachable (exit=$($Process.ExitCode)).`n--- $Name stderr ---`n$(Get-LogTail $ErrorLog)"};try{$r=Invoke-WebRequest -UseBasicParsing $Url -TimeoutSec 1;if($r.StatusCode-ge 200-and $r.StatusCode-lt 500){return}}catch{};Start-Sleep -Milliseconds 250}
