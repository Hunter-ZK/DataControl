...
def build_poc_server():
    ...
    if os.getenv("AGENT3_MCP_POC_MODE") != "1":
        raise RuntimeError("Static MCP identity is PoC-only. Set AGENT3_MCP_POC_MODE=1 for local P3 verification.")
    try:
    ...
    adapter = MCPToolAdapter(core, static_authz_provider(AuthzContext(principal="datacontrol-local", roles=("poc",), purpose="p3-local")))
    mcp = FastMCP("agent3", host="127.0.0.1", port=8900)
    for tool in (adapter.search_tables, adapter.get_schema, adapter.get_semantic_model, adapter.resolve_metric, adapter.search_verified_sql, adapter.validate_sql, adapter.explain_sql, adapter.compile_query, adapter.submit_ddl):
        mcp.tool()(tool)
    return mcp
