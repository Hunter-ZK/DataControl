<template>
  <div class="dc-page"><div class="dc-container agent-page">
    <section class="agent-hero dc-card">
      <div><span class="dc-eyebrow">TRUSTED DATA Q&A</span><h1>智能问数</h1><p>由 dsh 驱动模型循环，通过 Agent3 的只读工具检索资产、理解口径并生成可信 SQL。当前阶段不会执行 SQL。</p></div>
      <div class="runtime-pill" :class="{ready:status?.ready}"><span></span><div><b>{{status?.ready?'Agent 已就绪':'Agent 未就绪'}}</b><small>{{status?.ready?`${status.provider} · ${status.model}`:(status?.reason||'正在检测运行环境')}}</small></div></div>
    </section>

    <div class="agent-layout">
      <section class="dc-card chat-panel">
        <div class="messages" ref="messageBox">
          <article class="message assistant"><div class="avatar"><Sparkles :size="17"/></div><div><b>DataControl Agent</b><p>你可以问我“最近一期贷款余额应该使用哪张表？”、“region_code 对应什么标准？”或者“生成各地区贷款余额汇总 SQL”。</p></div></article>
          ...
          <article v-if="loading" class="message assistant"><div class="avatar"><LoaderCircle class="spin" :size="17"/></div><div><b>正在处理</b><p>正在通过 dsh / Agent3 工具链处理请求，不展示隐藏思维过程。</p></div></article>
        </div>
        <div class="composer">
      ...
      <aside class="agent-side">
        <section class="dc-card side-card"><span class="dc-eyebrow">RUNTIME</span><h3>运行链路</h3><div class="runtime-flow"><span>DataControl</span><ArrowDown/><span>dsh ACP</span><ArrowDown/><span>Agent3 MCP</span><ArrowDown/><span>SQL Validation</span></div></section>
        <section class="dc-card side-card"><span class="dc-eyebrow">STATUS</span><h3>环境状态</h3><dl><div><dt>dsh</dt><dd>{{status?.dshExecutable?'已发现':'未发现'}}</dd></div><div><dt>Agent3 MCP</dt><dd>{{status?.agent3McpConfigured?'已配置':'未配置'}}</dd></div><div><dt>SQL 执行</dt><dd>关闭</dd></div><div><dt>隐藏推理</dt><dd>不展示</dd></div></dl></section>
        <section v-if="!status?.ready" class="dc-card side-card blocker"><TriangleAlert :size="18"/><div><b>当前阻塞</b><p>{{status?.reason}}</p><small>Portal 不会伪造 Agent 成功结果。配置完成前，问数入口保持不可发送状态。</small></div></section>
      </aside>
    </div>
  </div></div>
</template>
