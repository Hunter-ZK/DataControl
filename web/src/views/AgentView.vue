<template>
  <div class="dc-page"><div class="dc-container agent-page">
    <section class="agent-hero dc-card">
      <div><span class="dc-eyebrow">TRUSTED DATA Q&A</span><h1>智能问数</h1><p>DataAgent 作为 DataControl 仓库内的独立子系统运行：dsh 负责 Agent Loop，MCP Adapter 调用 Agent3 Core，Portal 只负责产品入口和会话交付。当前阶段不会执行 SQL。</p></div>
    ...
    <div class="agent-layout">
      ...
      <aside class="agent-side">
        ...
        <section class="dc-card side-card"><span class="dc-eyebrow">STATUS</span><h3>环境状态</h3><dl><div><dt>运行模式</dt><dd>{{status?.mode||'—'}}</dd></div><div><dt>代码归属</dt><dd>{{status?.source||'DataControl/agent'}}</dd></div><div><dt>Agent 已迁入</dt><dd>{{status?.integrated?'是':'否'}}</dd></div><div><dt>本地服务</dt><dd>{{status?.serviceReachable?'已启动':'未启动'}}</dd></div><div><dt>SQL 执行</dt><dd>关闭</dd></div><div><dt>隐藏推理</dt><dd>不展示</dd></div></dl></section>
        <section v-if="!status?.ready" class="dc-card side-card blocker"><TriangleAlert :size="18"/><div><b>当前阻塞</b><p>{{status?.reason}}</p><small>DataAgent-dsh 只作为迁移基线，不是运行时外部依赖。仓库内 Agent 子系统完成迁移并启动前，不伪造问数结果。</small></div></section>
      </aside>
    </div>
  </div></div>
</template>
