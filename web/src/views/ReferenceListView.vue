<template>
  <div class="dc-page"><div class="dc-container">
    ...
    <section class="dc-card ref-panel"><div class="ref-head"><b>{{rows.length}} 条资产</b><span>{{config.note}}</span></div><div v-if="rows.length" class="ref-grid"><article v-for="row in rows" :key="keyOf(row)" class="ref-card"><div class="ref-kicker"><span class="dc-chip">{{config.badge}}</span><span>{{statusOf(row)}}</span></div><h3>{{titleOf(row)}}</h3><code class="dc-tech">{{codeOf(row)}}</code><p>{{descOf(row)}}</p><div class="ref-meta" v-if="kind==='metrics'"><span>聚合：{{row.aggregation||'—'}}</span><span>来源：{{row.sourceDatasetId||'—'}}</span></div><div class="ref-meta" v-else-if="kind==='stat-systems'"><span>{{row.issuer||'—'}}</span><span>{{row.documentNo||'—'}}</span></div><div class="ref-meta" v-else-if="kind==='word-roots'"><span>{{row.enFull||'—'}}</span><span>{{row.synonyms||'—'}}</span></div></article></div><div v-else class="dc-empty">暂无匹配资产。</div></section>
  </div></div>
</template>
