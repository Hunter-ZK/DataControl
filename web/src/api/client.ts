...
export interface AgentStatus{ready:boolean;mode:string;source:string;gatewayUrl:string;provider:string;model:string;sourceMigrated:boolean;integrated:boolean;serviceReachable:boolean;nextGate?:string|null;reason?:string|null;sqlExecutionEnabled:boolean;hiddenReasoningExposed:boolean}
export interface AgentResult{sessionId?:string;answer:string;stopReason?:string|null;events?:Array<Record<string,unknown>>;sqlExecuted:boolean}
...
export const agentApi={status:()=>get<AgentStatus>('/agent/status'),query:(question:string)=>post<AgentResult>('/agent/query',{question})}
