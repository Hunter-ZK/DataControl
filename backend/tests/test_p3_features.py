...
def test_unified_search_returns_facets_and_reference_assets():
    ...
    assert data["total"] >= 1
    assert any(item["assetType"] in {"STANDARD", "CODE_TABLE", "COLUMN"} for item in data["items"])

    suggestion = client.get("/api/v1/search/suggest", params={"q": "region", "limit": 8})
    assert suggestion.status_code == 200
    assert len(suggestion.json()["data"]) >= 1


def test_relation_graph_path_and_impact_contract():
    graph = client.get("/api/v1/relations/graph/DS000001", params={"depth": 2, "direction": "both"})
    assert graph.status_code == 200
    graph_data = graph.json()["data"]
    assert graph_data["centerAssetId"] == "DS000001"
    assert any(node["assetId"] == "DS000001" for node in graph_data["nodes"])
    ...
    assert path.json()["data"]["hopCount"] == 3

    impact = client.get("/api/v1/relations/impact/DS000001", params={"depth": 3})
